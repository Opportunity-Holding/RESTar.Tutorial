...
Now, let's try getting some Excel files. For this, we set the `Accept` header to `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`. For Postman, click the arrow to the right of the **Send** button, and select **Send and Download**. This will save the Excel file to disk. Now try the requests above again!
